...
export default function Event({event, ...params}) {
  const { slug } = params;
  const {
    frontmatter: { title, description, media, mediaType, date, author = '', authorURL = '' },
    content
  } = event;
            ...
            <div className="flex items-center mt-6 sm:mt-8">
              ...
              <div className="text-sm lg:text-[15px] flex items-center">
                <span className="text-gray-500 italic ml-4">
                  Event added by <Link href={authorURL} className="text-blue-500 hover:text-blue-400 underline">{author}</Link>
                </span>
              </div>
            </div>
          </div>
