...
export default function Opinion({opinion, ...params}) {
  const { slug } = params;
  const {
    frontmatter: { title, description, media, date, author = '', authorURL = '' },
    content
  } = opinion;
  let pageUrl = `${siteConfig.baseURL.replace(/\/$|$/, '/')}opinions/${slug}`
  ...
  return (
    <article className="pb-12 sm:pb-16 lg:pb-24 bg-gray-50">
      ...
      <header>
        ...
        <div className="px-5 lg:px-0">
          ...
          <div className="pt-10 mx-auto text-lg max-w-prose border-gray-300/70 sm:pt-16">
            <Link href="/opinions" className="relative text-sm font-semibold tracking-widest text-red-700 uppercase duration-300 ease-in-out transition-color hover:text-red-500">
              Back to opinions
            </Link>
          </div>
          <div className="pt-4 pb-8 mx-auto mb-8 text-lg border-b max-w-prose border-gray-300/70 sm:pt-6">
            ...
            <div className="flex items-center mt-6 sm:mt-8">
              ...
              <div className="text-sm lg:text-[15px] flex items-center">
                <span className="text-gray-500 italic ml-4">
                  Opinion written by <Link href={authorURL} className="text-blue-500 hover:text-blue-400 underline">{author}</Link>
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="px-5 lg:px-0">
        ...
        <div className="mx-auto prose sm:prose-lg hover:prose-a:text-red-700 prose-a:duration-300 prose-a:ease-in-out prose-a:transition prose-img:rounded-xl" dangerouslySetInnerHTML={{ __html: marked.parse(content) }}>
        </div>
